      ...
      <div className="flex-1 px-5 pt-4 space-y-3">
        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          <DialogTrigger asChild>
            <button className="w-full h-14 rounded-full bg-squad-pink border-2 border-foreground font-display text-sm font-bold uppercase tracking-[0.15em] text-foreground"
          ...
          <DialogContent className="border-2 border-foreground rounded-2xl" style={{ boxShadow: '6px 6px 0px 0px hsl(0 0% 8%)' }}>
            <DialogHeader><DialogTitle className="font-display text-base uppercase tracking-[0.15em] font-bold">Create Squad</DialogTitle></DialogHeader>
        ...
        <Dialog open={joinOpen} onOpenChange={setJoinOpen}>
          <DialogTrigger asChild>
            <button className="w-full h-14 rounded-full bg-squad-lavender border-2 border-foreground font-display text-sm font-bold uppercase tracking-[0.15em] text-foreground"
              style={{ boxShadow: shadow }}>Join with Code</button>
          </DialogTrigger>
          <DialogContent className="border-2 border-foreground rounded-2xl" style={{ boxShadow: '6px 6px 0px 0px hsl(0 0% 8%)' }}>
            <DialogHeader><DialogTitle className="font-display text-base uppercase tracking-[0.15em] font-bold">Join Squad</DialogTitle></DialogHeader>
            <div className="rounded-full" style={{ boxShadow: shadowSm }}>
        ...
        <div className="mt-2">
          <h2 className="mb-3 text-[11px] font-display uppercase tracking-[0.15em] font-bold text-foreground">Current Squads ({groups.length})</h2>
          {loading ? (
            <div className="py-8 text-center text-muted-foreground font-display text-sm uppercase">Loading...</div>
          ) : groups.length === 0 ? (
            <div className="py-8 text-center text-muted-foreground font-display text-sm uppercase">No squads yet</div>
          ) : (
            <div className="space-y-3">
              {groups.map((group) => (
                <button key={group.id} onClick={() => navigate(`/squads/${group.id}`)}
                  className="flex w-full items-center gap-3 rounded-2xl border-2 border-foreground bg-card p-4 text-left" style={{ boxShadow: shadowSm }}>
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl border-2 border-foreground bg-squad-pink"
                    style={{ boxShadow: '2px 2px 0px 0px hsl(0 0% 8%)' }}>
                    <Users className="h-5 w-5 text-foreground" strokeWidth={2.5} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-display text-sm font-bold uppercase tracking-[0.08em] text-foreground">{group.group_name}</h3>
                    <p className="text-[11px] font-display uppercase tracking-[0.08em] text-muted-foreground">{group.memberCount} Members</p>
                  </div>
                  <div className="flex gap-1.5">
                    <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(group.invite_code); toast.success('Invite code copied!'); }}
                      className="flex h-9 w-9 items-center justify-center rounded-lg border-2 border-foreground bg-squad-pink"
                      style={{ boxShadow: '2px 2px 0px 0px hsl(0 0% 8%)' }}>
                    ...
                    <button onClick={(e) => e.stopPropagation()}
                      className="flex h-9 w-9 items-center justify-center rounded-lg border-2 border-foreground bg-squad-pink"
                      style={{ boxShadow: '2px 2px 0px 0px hsl(0 0% 8%)' }}>
                      <Settings className="h-4 w-4 text-foreground" strokeWidth={2.5} />
                    </button>
                  </div>
                </button>
              ))}
