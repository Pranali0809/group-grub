      ...
      <div className="flex-1 px-5 pt-4 space-y-3">
        ...
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) resetForm(); }}>
          <DialogTrigger asChild>
            <button
              className="flex w-full items-center justify-center gap-2 h-12 rounded-full bg-secondary border-2 border-foreground font-display text-[11px] font-bold uppercase tracking-[0.2em] text-secondary-foreground"
          ...
          <DialogContent className="max-h-[85vh] overflow-y-auto border-2 border-foreground rounded-2xl" style={{ boxShadow: '6px 6px 0px 0px hsl(0 0% 8%)' }}>
            <DialogHeader>
              <DialogTitle className="font-display text-sm uppercase tracking-[0.2em] font-bold">Add Restaurant</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div>
                <label className="mb-2 block text-[9px] font-display uppercase tracking-[0.2em] font-bold text-foreground">Restaurant Name *</label>
              ...
              <div>
                <label className="mb-2 block text-[9px] font-display uppercase tracking-[0.2em] font-bold text-foreground">Link (Maps, Instagram, etc.)</label>
                <div className="rounded-full" style={{ boxShadow: shadowSm }}>
              ...
              <div>
                <label className="mb-2 block text-[9px] font-display uppercase tracking-[0.2em] font-bold text-foreground">Image</label>
                <div className="rounded-2xl" style={{ boxShadow: shadowSm }}>
                  <Input placeholder="Image URL (or upload below)" value={restaurantImage} onChange={(e) => setRestaurantImage(e.target.value)}
                    className="h-11 rounded-full border-2 border-foreground bg-card font-body text-xs px-5" />
                </div>
                <Input type="file" accept="image/*" className="mt-2 rounded-full border-2 border-foreground text-xs"
                  onChange={(e) => setImageFile(e.target.files?.[0] || null)} />
              </div>

              {/* Price */}
              <div>
                <label className="mb-2 block text-[9px] font-display uppercase tracking-[0.2em] font-bold text-foreground">Price</label>
                <div className="flex gap-2">
                  {priceOptions.map(p => (
                    <button key={p} type="button" onClick={() => setPriceCategory(prev => prev === p ? '' : p)}
                      className={`flex-1 rounded-full py-2.5 text-[10px] font-display uppercase tracking-[0.15em] font-bold border-2 border-foreground transition-colors ${
              ...
              {/* Cuisine */}
              <div>
                <label className="mb-2 block text-[9px] font-display uppercase tracking-[0.2em] font-bold text-foreground">Cuisine</label>
                {cuisineTag && (
                  <div className="mb-2 flex items-center gap-1">
                    <span className="inline-flex items-center gap-1 rounded-full bg-squad-pink-deep border-2 border-foreground px-3 py-1 text-[10px] font-display uppercase font-bold text-foreground">
                      {cuisineTag}
                  ...
                  <>
                    <div className="relative rounded-full" style={{ boxShadow: shadowSm }}>
                      ...
                      <Input placeholder="Search cuisine..." value={cuisineSearch} onChange={(e) => setCuisineSearch(e.target.value)}
                        className="h-11 rounded-full border-2 border-foreground bg-card pl-10 font-body text-xs" />
                    </div>
                    <div className="mt-2 flex flex-wrap gap-1.5 max-h-28 overflow-y-auto">
                      {filteredCuisines.map(c => (
                        <button key={c} type="button" onClick={() => { setCuisineTag(c); setCuisineSearch(''); }}
                          className="rounded-full border-2 border-foreground bg-card px-3 py-1 text-[9px] font-display uppercase font-bold text-foreground hover:bg-squad-pink transition-colors"
                      ...
                      {cuisineSearch && filteredCuisines.length === 0 && (
                        <button type="button" onClick={() => { setCuisineTag(cuisineSearch); setCuisineSearch(''); }}
                          className="rounded-full border-2 border-foreground bg-squad-pink px-3 py-1 text-[9px] font-display uppercase font-bold text-foreground">
                          + Add "{cuisineSearch}"
                        </button>
                      )}
                    </div>
              ...
              {/* Highlight */}
              <div>
                <label className="mb-2 block text-[9px] font-display uppercase tracking-[0.2em] font-bold text-foreground">Highlight</label>
                <div className="flex flex-wrap gap-2">
                  {highlightOptions.map(h => (
                    <button key={h} type="button" onClick={() => setHighlightTag(prev => prev === h ? '' : h)}
                      className={`rounded-full px-4 py-2 text-[10px] font-display uppercase tracking-[0.15em] font-bold border-2 border-foreground transition-colors ${
              ...
              {/* Custom Tags */}
              <div>
                <label className="mb-2 block text-[9px] font-display uppercase tracking-[0.2em] font-bold text-foreground">Custom Tags</label>
                <div className="rounded-full" style={{ boxShadow: shadowSm }}>
        ...
        {loading ? (
          ...
          <div className="py-8 text-center text-muted-foreground font-display text-xs uppercase">Your wishlist is empty</div>
        ) : (
            ...
            {items.map((item) => (
              <div key={item.id} className="overflow-hidden rounded-2xl border-2 border-foreground bg-card" style={{ boxShadow: shadowSm }}>
                {item.restaurant_image && (
                  <img src={item.restaurant_image} alt={item.restaurant_name} className="h-32 w-full object-cover" />
                )}
                <div className="p-3">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="mt-1 flex flex-wrap gap-1">
                        {item.cuisine_tag && (
                          <span className="rounded-full bg-squad-pink border border-foreground px-2 py-0.5 text-[8px] font-display uppercase font-bold text-foreground">{item.cuisine_tag}</span>
                        )}
                        {item.price_range && (
                          <span className="rounded-full bg-squad-lavender border border-foreground px-2 py-0.5 text-[8px] font-display uppercase font-bold text-foreground">{item.price_range}</span>
                        )}
                      </div>
                      {item.highlight_tag && (
                        <p className="mt-1 text-[9px] font-display uppercase tracking-[0.1em] text-muted-foreground">
