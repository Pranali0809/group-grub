    ...
    <div className="flex min-h-screen flex-col pb-20" style={{ backgroundColor: 'hsl(348 60% 95%)' }}>
      ...
      <div className="flex-1 px-5 pt-4 space-y-4">
        ...
        {winner && (
          <div className="rounded-2xl border-2 border-foreground bg-card p-5 text-center" style={{ boxShadow: shadow }}>
            ...
            <h2 className="mt-2 font-display text-xl font-bold uppercase italic text-foreground">The Winner!</h2>
            ...
            <div className="mt-3 flex items-center justify-center gap-2">
              ...
              <span className="font-display text-sm font-bold uppercase tracking-[0.1em] text-foreground">{winner.restaurant_name}</span>
            </div>
            ...
            <button onClick={() => { setSelectedRestaurant(winner); setHangoutOpen(true); }}
              className="mt-4 flex w-full items-center justify-center gap-2 h-11 rounded-full bg-squad-lavender border-2 border-foreground font-display text-[11px] font-bold uppercase tracking-[0.2em] text-foreground"
        ...
        <div className="space-y-2">
          {results.map((item, index) => (
            <div key={item.wishlist_item_id}
              className="flex items-center gap-3 rounded-2xl border-2 border-foreground bg-card p-3"
              style={{ boxShadow: shadowSm }}>
              ...
              {item.restaurant_image && (
                <img src={item.restaurant_image} alt="" className="h-10 w-10 rounded-lg border border-foreground object-cover" />
              )}
              <div className="flex-1 min-w-0">
                <h3 className="font-display text-[11px] font-bold uppercase tracking-[0.1em] text-foreground">{item.restaurant_name}</h3>
                <p className="text-[8px] font-display uppercase text-muted-foreground">Accept Votes</p>
              </div>
              <div className="text-right">
                <Star className="mx-auto h-4 w-4 text-squad-gold" />
                <p className="text-[9px] font-display font-bold text-foreground">{item.accept_count}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <Dialog open={hangoutOpen} onOpenChange={setHangoutOpen}>
        <DialogContent className="border-2 border-foreground rounded-2xl" style={{ boxShadow: '6px 6px 0px 0px hsl(0 0% 8%)' }}>
          <DialogHeader>
            <DialogTitle className="font-display text-sm uppercase tracking-[0.2em] font-bold">Plan Hangout</DialogTitle>
          </DialogHeader>
