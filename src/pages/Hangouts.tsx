    ...
    <div className="flex min-h-screen flex-col pb-20" style={{ backgroundColor: 'hsl(348 60% 95%)' }}>
      ...
      <div className="flex gap-2 px-5 pt-3">
        <button onClick={() => setTab('upcoming')}
          className={`rounded-full px-5 py-2 font-display text-[10px] uppercase tracking-[0.2em] font-bold border-2 border-foreground ${
            tab === 'upcoming' ? 'bg-squad-pink-deep text-foreground' : 'bg-card text-foreground'
          }`} style={{ boxShadow: '2px 2px 0px 0px hsl(0 0% 8%)' }}>
          Upcoming
        </button>
        <button onClick={() => setTab('history')}
          className={`rounded-full px-5 py-2 font-display text-[10px] uppercase tracking-[0.2em] font-bold border-2 border-foreground ${
            tab === 'history' ? 'bg-squad-pink-deep text-foreground' : 'bg-card text-foreground'
          }`} style={{ boxShadow: '2px 2px 0px 0px hsl(0 0% 8%)' }}>
          History
        </button>
      </div>
      ...
      <div className="flex-1 px-5 pt-3 space-y-3">
        {tab === 'upcoming' && (
          <h2 className="text-[9px] font-display uppercase tracking-[0.2em] font-bold text-foreground flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full bg-squad-success" />
            Upcoming Hangouts
          </h2>
        )}
        {tab === 'history' && (
        ...
        {loading ? (
          ...
          <div className="py-8 text-center text-muted-foreground font-display text-xs uppercase">
            ...
            <div key={hangout.id} className="overflow-hidden rounded-2xl border-2 border-foreground bg-card" style={{ boxShadow: shadowSm }}>
              ...
              <div className="border-t-2 border-foreground p-3">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-display text-[11px] font-bold uppercase tracking-[0.1em] text-foreground">{hangout.restaurant_name}</h3>
                    <p className="mt-0.5 flex items-center gap-1 text-[9px] font-display uppercase tracking-[0.1em] text-muted-foreground">
                  ...
                  {hangout.status === 'completed' && (
                    <span className="rounded-full bg-squad-success border border-foreground px-2 py-0.5 text-[8px] font-display uppercase font-bold text-card">Done</span>
                  )}
                </div>
                <div className="mt-2 flex gap-2">
                  {hangout.status === 'upcoming' && (
                    <>
                      <button className="flex items-center gap-1 rounded-full border-2 border-foreground bg-squad-pink px-3 py-1.5 text-[8px] font-display uppercase font-bold text-foreground"
                        style={{ boxShadow: '2px 2px 0px 0px hsl(0 0% 8%)' }}>
                        <Users className="h-3 w-3" /> People
                      </button>
                      <button onClick={() => navigate(`/hangouts/${hangout.id}/edit`)}
                        className="flex items-center gap-1 rounded-full border-2 border-foreground bg-squad-lavender px-3 py-1.5 text-[8px] font-display uppercase font-bold text-foreground"
                        style={{ boxShadow: '2px 2px 0px 0px hsl(0 0% 8%)' }}>
                  ...
                  {hangout.status === 'completed' && (
                    <>
                      <button onClick={() => navigate(`/hangouts/${hangout.id}/memories`)}
                        className="flex items-center gap-1 rounded-full border-2 border-foreground bg-squad-pink px-3 py-1.5 text-[8px] font-display uppercase font-bold text-foreground"
                        style={{ boxShadow: '2px 2px 0px 0px hsl(0 0% 8%)' }}>
