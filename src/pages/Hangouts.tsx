...
const Hangouts = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col pb-16" style={{ backgroundColor: 'hsl(348 60% 95%)' }}>
      <PageHeader title="Hangouts" leftIcon={<Sparkles className="h-4 w-4 text-foreground" strokeWidth={2.5} />} />
      ...
      <div className="flex-1 px-4 pt-2.5 space-y-2.5">
        <h2 className="text-[9px] font-display uppercase tracking-[0.2em] font-bold text-foreground flex items-center gap-1.5">
          <span className={`h-1.5 w-1.5 rounded-full ${tab === 'upcoming' ? 'bg-squad-success' : 'bg-muted-foreground'}`} />
        ...
        {loading ? (
          ...
          <div className="py-8 text-center text-muted-foreground font-display text-[10px] uppercase tracking-[0.15em]">
              ...
              <div className="border-t-2 border-foreground p-2.5">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-0.5 flex items-center gap-1 text-[9px] font-display uppercase tracking-[0.08em] text-muted-foreground">
                      <Users className="h-3 w-3" /> Hosted by {(hangout.profiles as any)?.name || 'Unknown'}
                    </p>
                  </div>
                  {hangout.status === 'completed' && (
                    <span className="rounded-full bg-squad-success border-2 border-foreground px-2 py-0.5 text-[8px] font-display uppercase font-bold text-card">Done</span>
                  )}
                </div>
                <div className="mt-2 flex gap-1.5">
                  ...
                  {hangout.status === 'completed' && (
                    <>
                      ...
                      <button onClick={() => navigate(`/hangouts/${hangout.id}/memories`)}
                        className="flex items-center gap-1 rounded-full border-2 border-foreground bg-squad-lavender px-3 py-1.5 text-[9px] font-display uppercase font-bold text-foreground"
                        style={{ boxShadow: shadowSm }}><Eye className="h-3 w-3" /> View</button>
                    </>
                  )}
                </div>
              </div>
