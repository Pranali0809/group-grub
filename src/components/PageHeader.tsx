...
const shadow = '2px 2px 0px 0px hsl(0 0% 8%)';

const PageHeader = ({ title, showBack = false, rightIcon, onRightClick, leftIcon }: PageHeaderProps) => {
  const navigate = useNavigate();

  return (
    <header className="sticky top-0 z-40 flex items-center justify-between px-4 py-3" style={{ backgroundColor: 'hsl(348 60% 95%)' }}>
      {showBack ? (
        <button
          onClick={() => navigate(-1)}
